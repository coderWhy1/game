...
const togglePause = (v) => store.setState({ paused: v ?? !store.state.paused });
const toggleSound = (v) => store.setState({ soundEnabled: v ?? !store.state.soundEnabled });
const toggleMenu = (v) => store.setState({ menuOpen: v ?? !store.state.menuOpen });
...
function renderApp(state) {
	...
	appNodes.menu.classList.toggle('show', state.menuOpen);
...
// Menu close button
document.querySelector('.menu__close-btn')?.addEventListener('click', (e) => {
	e.stopPropagation?.();
	toggleMenu(false);
});
...
document.addEventListener('click', (e) => {
	...
	if (menu?.classList.contains('show') && !menu.contains(e.target) && !settingsBtn?.contains(e.target)) {
		toggleMenu(false);
	}
});
...
function init() {
	...
	// shell type with Chinese labels
	...
	let options = '';
	shellNames.forEach(opt => options += `<option value="${opt}">${shellTypeLabels[opt] || opt}</option>`);
...
function launchShellFromConfig(event) {
	...
	shell.launch(
		event ? event.x / w : fitShellPositionInBoundsH(Math.random()),
		event ? 1 - event.y / h : fitShellPositionInBoundsV(Math.random())
	);
}
